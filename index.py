...
def get_movie_detail():
    ...
    try:
        ...
        print ("detalle de la pelicula", movie_detail)
        response =  """
            Title : {0}
            Released: {1}
            Actors: {2}
...
def detect_intent_texts(project_id, session_id, text, language_code):
    ...
    if text:
        ...
        print ("respuesta del df", response)
...
def send_message():
    ...
    print ("identificador del socket", socketId)
    message = request.form['message']
    project_id = os.getenv('DIALOGFLOW_PROJECT_ID')
    fulfillment_text = detect_intent_texts(project_id, "unique", message, 'es')
    response_text = { "message":  fulfillment_text }
    
    print ("texto con la respuesta al mensaje:", response_text)
    pusher_client.trigger('bot-ml', 'new_message',
